...
impl Interval {
    ...
    pub fn new(dur: Duration, remote: &Remote) -> io::Result<Interval> {
        Interval::new_at(Instant::now() + dur, dur, remote)
    }
    ...
    pub fn new_at(at: Instant, dur: Duration, remote: &Remote)
        -> io::Result<Interval>
    {
        Ok(Interval {
            token: try!(TimeoutToken::new(at, remote)),
            next: at,
            interval: dur,
            handle: remote.clone(),
        })
